...
// FIX CRÍTICO: Se utiliza {$set: req.body} para asegurar que los arrays anidados (como 'widgets') 
// se sobrescriban por completo, corrigiendo problemas de actualización.
router.put('/periods/:id', getValidId, async (req, res) => { 
  try {
    // Usamos $set para forzar a Mongoose a sobrescribir los campos que son arrays
    const updatedPeriod = await Period.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true, runValidators: true });

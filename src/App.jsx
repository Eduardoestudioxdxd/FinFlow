...
function App() {
  ...
  const fetchData = async () => {
      ...
      try {
          ...
          if (periodsData.length > 0 && !dashboardPeriodId) { setDashboardPeriodId(periodsData[periodsData.length - 1]._id); }
      } catch (error) { console.error("Error cargando datos:", error); } finally { setIsLoading(false); }
  };
  ...
  const handleSaveData = async (data) => {
    setIsLoading(true);
    try {
        if (data.type === 'card') { const newCard = { nombre: data.name, digitos: data.digitos, tipo: data.tipo, gasto: data.amount, limit: data.limit, color: data.color }; const savedCard = await api.createCard(newCard); setTarjetas([...tarjetas, savedCard]); }
        else if (data.type === 'edit-card') { const updatedCard = { nombre: data.name, digitos: data.digitos, tipo: data.tipo, gasto: data.amount, limit: data.limit, color: data.color }; const res = await api.updateCard(data.id, updatedCard); setTarjetas(tarjetas.map(t => t._id === data.id ? res : t)); }
        else if (data.type === 'budget') { const newPeriod = { name: data.name, budget: data.amount, spent: 0, color: data.color, movements: [] }; const savedPeriod = await api.createPeriod(newPeriod); setPeriods([...periods, savedPeriod]); setDashboardPeriodId(savedPeriod._id); setActiveTab('quincenas'); }
        else if (data.type === 'edit-budget') { const original = periods.find(p => p._id === data.id); const updatedData = { ...original, name: data.name, budget: data.amount, color: data.color }; const res = await api.updatePeriod(data.id, updatedData); setPeriods(periods.map(p => p._id === data.id ? res : p)); } 
        else {
    ...
    } catch (error) { console.error("Error guardando:", error); alert("Hubo un error al guardar los datos en la nube."); } finally { setIsLoading(false); setMenuAbierto(false); setMenuView('main'); setEditingData(null); }
  };
            ...
            {activeTab === 'quincenas' && (
                <>
                    ...
                    {selectedPeriodId && <PeriodDetail 
                        period={periods.find(p => p._id === selectedPeriodId)} onBack={() => setSelectedPeriodId(null)} onEdit={() => openEditModal(periods.find(p => p._id === selectedPeriodId))} onDeleteMovement={(movId) => deleteMovement(selectedPeriodId, movId)} widgets={periods.find(p => p._id === selectedPeriodId).widgets || []} moveWidget={(idx, dir) => movePeriodWidget(selectedPeriodId, idx, dir)} removeWidget={(wId) => removePeriodWidget(selectedPeriodId, wId)} updateWidgetTitle={() => {}} 
                    />}
                </>
            )}

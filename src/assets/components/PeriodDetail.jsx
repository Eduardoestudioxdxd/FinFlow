...
// AÑADIMOS "onDeleteMovement" A LAS PROPS
function PeriodDetail({ period, onBack, widgets, moveWidget, removeWidget, updateWidgetTitle, onEdit, onDeleteMovement }) {
               ...
               <div className="flex flex-col md:flex-row gap-8 items-center h-auto md:h-64">
                  <div className="w-full md:w-1/2 h-56 relative">
                      <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie data={dataCategorias} innerRadius={65} outerRadius={80} paddingAngle={5} dataKey="value" stroke="none">
                              {dataCategorias.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                            </Pie>
                            <Tooltip formatter={(value) => `$${value}`} contentStyle={{backgroundColor: '#1e293b', border:'none', borderRadius:'12px', color:'#fff'}} itemStyle={{color: '#fff'}}/>
                          </PieChart>
                      </ResponsiveContainer>
                        ...
                        period.movements.map((mov) => {
                            ...
                            return (
                                <div key={mov.id} className="relative flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-900/50 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-xl transition-colors border border-transparent hover:border-emerald-500/20 group/item">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className={`font-bold text-sm ${isIncome ? 'text-emerald-500' : 'text-slate-700 dark:text-slate-300'}`}>
                                            {isIncome ? '+' : '-'}${mov.amount.toFixed(2)}
                                        </span>
                                        {/* BOTÓN DE BORRAR (SOLO VISIBLE AL HACER HOVER) */}
                                        <button 
                                            onClick={() => onDeleteMovement(mov.id)}
                                            className="opacity-0 group-hover/item:opacity-100 p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-all"
